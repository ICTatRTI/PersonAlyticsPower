...
# TODO how many of the items in private can be moved to ICTdesign because all
...
polyICT <- R6::R6Class("polyICT",
                       ...
                       public  = list(
                         ...
                         (
                           ...
                           randFxMean        = list(phase1=list(group1=c(0,0),
                                                                group2=c(0,0)),
                         ...
                         {
                           # general input validation and expansion
                           polyInputs <- checkPolyICT(n, randFxMean, phases,
                                                      randFxCorMat, randFxVar)
                           n                <- polyInputs$n
                           randFxCorMat     <- polyInputs$randFxCorMat
                           randFxCovMat     <- polyInputs$randFxCovMat
                           ...
                           unStdRandFxMean  <- polyInputs$unStdRandFxMean
                           rm(polyInputs)

                           if(!all(names(phases)==phaseNames)) names(phases) <- phaseNames
                           ...
                           designMat <- getICTdesign(phases, phaseNames,
                                                     maxRandFx, 'polyICT')
                         ...
                         makeData = function(seed=123, ymean=NULL, yvar=NULL)
                         {
                           ...
                           for(p in seq_along(self$phaseNames))
                           {
                             ...
                             for(g in seq_along(self$groupNames))
                             {
                               ...
                               Sigma <- self$randFxCovMat[[thisp]][[thisg]]
                               ...
                               data[[d]] <- polyData(seeds[p,g], n, nObs,
                                                      mu, Sigma, self, dM,
                                                      rFxVr, propErrVar,
                                                      yMean, ySD, thisp,
                                                      thisg); d <- d + 1
...
checkPolyICT <- function(n, randFxMean, phases, randFxCorMat, randFxVar)
{
  ...
  effectSizeLengths <- lapply(randFxMean, length)
  ...
  if(is.list(randFxCorMat))
  {
    ...
    {
      if(! all.equal(unlist(randFxCorMatLengths), unlist(effectSizeLengths)) )
      {
